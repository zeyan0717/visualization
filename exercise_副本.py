...
def load_and_clean_data(path: str):
    ...
    # Convert key columns to numeric, coerce errors
    ...
    df_clean = df_raw.dropna(subset=['calories', 'protein'], how='all')
    return df_clean
...
min_cal = int(df['calories'].min() or 0)
max_cal = int(df['calories'].max() or 0)
...
min_pro = float(df['protein'].min() or 0)
max_pro = float(df['protein'].max() or 0)
...
min_sug = float(df['sugars'].min() or 0)
max_sug = float(df['sugars'].max() or 0)
...
if page == "Nutrition Analysis":
    ...
    with tab1:
        ...
        if selected_nutrients and not filtered_df.empty:
            ...
            st.warning("No data available for selected filters and nutrients.")
    ...
    with tab2:
        ...
        if selected_nutrients and not filtered_df.empty:
            ...
            st.warning("Select nutrients and ensure filtered data is not empty.")
...
# --- Page: Food Clustering ---
    ...
    if selected_nutrients and not filtered_df.empty:
        ...
        data_vals = filtered_df[selected_nutrients].dropna()
        scaler = MinMaxScaler()
...
# --- Page: Meal Planner ---
    ...
    st.write(totals)
    tp = st.number_input(
        "Protein target (g)", min_value=0.0, max_value=200.0, value=50.0,
        step=0.1, format="%.1f",
        help="Set your protein target for the meal."
    )
    tc = st.number_input(
        "Carbs target (g)", min_value=0.0, max_value=400.0, value=200.0,
        step=0.1, format="%.1f",
        help="Set your carbohydrates target for the meal."
    )
    tf = st.number_input(
        "Fat target (g)", min_value=0.0, max_value=100.0, value=30.0,
        step=0.1, format="%.1f",
...
# --- Page: Nutrition Network ---
    ...
    if selected_nutrients and len(selected_nutrients) >= 2 and not filtered_df.empty:
        ...
        st.warning("Select at least two nutrients and ensure filtered data is not empty.")
